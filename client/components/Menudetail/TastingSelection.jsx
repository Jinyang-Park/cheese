...
import { setTaste } from '../../redux/modules/ReservationsCakeDetail';
...
function TastingSelection() {
  ...
  const handleClickTast = (e) => {
    ...
    if (tastes.includes(selected)) {
      // 만약 목록에 있다면 해당 테이스팅을 목록에서 제거한다.
      // 현재 클릭한 테이스팅과 다른 모든 테이스팅만 새로운 배열을 만듬
      setTastes((prev) => prev.filter((notTasting) => notTasting !== selected));
    } else {
      if (tastes.length >= 3) {
            ...
            return (
              <CakedeTastingSelectionBtn
                ...
                className={
                  ...
                  tastes.includes(tast.Tasting) ? 'active' : ''
                }
